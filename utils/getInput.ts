 ...
 */
export default async function getInput(year: number, day: number): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch puzzle input: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}